...
async def get_bilibili_subtitle(
    ...
    """
    ctx.log("info",f"Received request for URL: {url}, lang: {preferred_lang}, format: {output_format}")
    ...
    try:
        ...
        if page and isinstance(pages_info, list) and len(pages_info) >= page:
            ...
            if 0 < page <= len(pages_info):
                 ...
                 if cid:
                     ...
                     ctx.log("warning",f"Page {page} found in 'pages' list, but 'cid' key is missing for that page.")
                     # Fallback to default cid if specific page cid is missing
                     cid = info.get('cid')
            else:
                 ctx.log("warning",f"Invalid page number {page} for video with {len(pages_info)} pages. Falling back to default page.")
                 cid = info.get('cid') # Fallback to the default cid if page is out of range
        else:
            if page:
                 ctx.log("warning",f"Page {page} requested but video seems to be single-part or page info missing/invalid. Using default cid.")
            cid = info.get('cid') # Default cid for single-part videos or if page not specified/found
            if cid:
        ...
        for sub in available_subtitles:
            if sub.get('lan') == preferred_lang:
                ...
                ctx.log("info",f"Found exact match for preferred language: {found_lang}")
                break

        # If exact match not found, try finding *any* subtitle (prioritizing non-AI)
        if not subtitle_url:
            ctx.log("warning",f"Preferred language '{preferred_lang}' not found. Searching for alternatives.")
            # Try non-AI first
            for sub in available_subtitles:
        ...
        ctx.log("info",f"Fetching subtitle content from: {subtitle_url} (Language: {found_lang})")
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.get(subtitle_url, headers=headers, follow_redirects=True)
        ...
        if output_format == "timestamped":
            for item in body:
                ...
                formatted_subtitle += f"{int(end_h):02}:{int(end_m):02}:{int(end_s):02}.{end_ms:03}\n"
        ...
        ctx.log("error",f"HTTP error fetching subtitle content: {e.response.status_code} for URL {e.request.url}")
        ...
        return f"Error fetching subtitle content: {error_details}"
         ...
         ctx.log("error",f"Network error fetching subtitle content for URL {e.request.url}: {e}")
